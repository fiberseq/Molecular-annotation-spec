...
## Results (total bytes over all 46; `agree` = samples matching the row's direction)
...
- **Container decides the sign, unanimously.** On every tag row the per-sample
  direction is 46/46: split is bigger in BAM on all 46 samples, and smaller in CRAM on
  all 46. split wins only under CRAM's per-series codecs, which pack the homogeneous
  `AL` integer array and the starts column far better than interleaved `start-len` text.
